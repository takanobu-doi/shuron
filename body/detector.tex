...
2次元平面でドリフトしてきた電子を読み出すことで，3次元的にトラックを検出できる．
...
\begin{figure}
  ...
  \caption{TPC によりトラックを検出するときのイメージ．}
  \label{fig::drift_electrons}
\end{figure}
\begin{figure}
  \centering
  \includegraphics[clip, width=0.8\columnwidth]{MAIKo2.eps}
...
\subsection{読み出し領域}
\label{sec::mu-pic}
図\ref{fig::MAIKo_view}中でanode strip が$x$軸，cathode strip が$z$軸と平行になるように$\mu$-PICが配置されている．
GEMと$\mu$-PICにより増幅された電子をanode strip とcathode strip により読み出すことで，
...
cathode strip に垂直な面 ($xy$平面) に射影されたcathode image の2つの画像が出力される．
...
また，anode strip，cathode strip ともに32~chごとにまとめて信号を波形として\SI{25}{\mega\hertz}でFADC で取得している．
...
\begin{figure}
  ...
  \caption[FADCで取得された$\mu$-PICの波形の一例．]
          ...
          この波形は\isoButaneHydro を検出ガスとして用いた際のものである．}
...
\begin{table}
  ...
  \caption[ガスの混合パターン，圧力，$dE/dx$．]
          {ガスの混合パターン，圧力，$dE/dx$．
            ...
            落とすエネルギーを表す．
            電場はMagboltz による計算で\SI{0.014}{\milli\metre\per\nano\second}となる値である．}
  \label{tab::mixture}
  \begin{tabular}{ccccc}
    \toprule
    gas &
...
に調整する必要がある．
Magboltz~\cite{magboltz} によって求めたドリフト電場とドリフトスピードの関係を図\ref{fig::drift_v_magboltz}に示す．
...
そのため，図\ref{fig::diffusion-image}の右のようにディフュージョンの効果が小さいことが望まれる．
\begin{figure}
  \centering
...
Ref~\cite{leo}によるとドリフト電場がない場合のディフュージョンは以下のように理解できる．
電子は熱運動により発生点から拡散する．
