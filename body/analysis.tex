...
そのため，anode image, cathode image のclock にドリフト速度をかけることで$y$座標を求めることができる．
このようにして決定したanode image, cathode image の座標を合わせることで，
3次元の座標を求めることができる．

散乱点と停止点の座標から粒子が飛行した方向ベクトルと距離が決定される．
粒子の種類が分かれば，飛行距離から運動エネルギーが決まる．
...
\begin{table}
  ...
  \begin{tabular}{ccc}
    \toprule
    gas & Ex (\si{\mega\electronvolt}) & $\sigma_{\text{Ex}}$ (\si{\kilo\electronvolt}) \\
    \midrule
    \Methane  & 7.63 & $49.1$ \\
...
よって，検出ガスには\SI{100}{\hecto\pascal}の\isoButaneHydro が最善であると判明した．
\begin{table}
  \centering
