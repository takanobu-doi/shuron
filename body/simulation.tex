...
\begin{align}
  \frac{\Delta z}{\SIU{0.4}{\milli\metre}} & = \Delta strip \label{eq::deltaz}\\
  \frac{\Delta y}{drift\_v} & = \Delta t \label{eq::deltay}
\end{align}
式 (\ref{eq::deltay_deltaz}, \ref{eq::deltaz}, \ref{eq::deltay}) より
\begin{equation}
  drift\_v = \frac{\tan(\ang{30})~\Delta strip\times\SIU{0.4}{\milli\metre}}{\Delta t}
\end{equation}
とドリフトスピードが求まる．
...
\begin{table}
  ...
  \begin{tabular}{cccc}
    \toprule
    gas & ドリフト電場 (\siu{\volt/\milli\metre}) & 実測 (\siu{\milli\metre/\nano\second}) & Magboltz (\siu{\milli\metre/\nano\second})\\
    \midrule
...
\Methane のみ\SIU{50}{\hecto\pascal} とその他のガスと比較して圧力が半分であるため，不純物の影響が大きく出ていると考えられる．
...
\begin{table}
  ...
  \begin{tabular}{cccc}
    \toprule
    gas & W (\siu{\electronvolt}) & energy loss (\siu{\mega\electronvolt}) & $N_{\rm e}$\\
    \midrule
...
入力インピーダンス\SIU{50}{\ohm}で電流値を電圧値に変換して取得している．
よって，式 (\ref{eq::N'e})で求めることができる．
$e$は電荷素量である．
\begin{equation}
  N'_{\rm e} = \frac{\int V (t) dt}{ 50 \times 800 \times \siu{\elementarycharge}}
  \label{eq::N'e}
\end{equation}
...
\begin{enumerate}
\item
  ${}^{12}{\rm C}$が\SIU{14}{\mega\electronvolt}の中性子との散乱によりHoyle状態に励起させる．
